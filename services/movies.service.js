...
export async function updateMovieByID(id, updateRequest) {
  return await client.db("Ragav").collection("movies").updateOne({ _id: ObjectId(id)}, { $set: updateRequest });
}
export async function deleteMovieByID(id) {
  return await client.db("Ragav").collection("movies").deleteOne({ _id: ObjectId(id)});
}
...
export async function getMovieByID(id) {
  // console.log("****", id)
  return await client.db("Ragav").collection("movies").findOne({ _id : ObjectId(id) });
}
